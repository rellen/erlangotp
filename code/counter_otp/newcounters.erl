...
-export([create/1, delete/1, increment/1, increment/2, decrement/1, decrement/2, count/1, reset/1]).
...
count (Name) ->
    try
        {ok, {Pid, Count}} = counter_store:lookup(Name),
        {ok, Count}
    catch
        _Class:_Exception ->
            {error, not_found}
...
increment_impl(Name, N) ->
    try
        {ok, {Pid, _}} = counter_store:lookup(Name),
